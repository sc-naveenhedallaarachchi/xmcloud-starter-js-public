...
const locale = new LocaleProxy({
  ...
  // This is an important performance consideration since the proxy runs on every matched request.
  // In multilanguage scenarios, we need locale handling to always run first so locale is set correctly for the rest of the chain.
  skip: () => false,
});
